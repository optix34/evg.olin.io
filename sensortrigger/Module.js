/**
 * Позиция по меткам – исправленная версия (ошибка Invalid LatLng)
 */
Ext.define('Store.sensortrigger.Module', {
    ...
    loadTriggerPoints: function() {
        ...
        if (stored) {
            try {
                ...
                // Отфильтровываем невалидные точки
                var validPoints = points.filter(function(p) {
                    return p.sensorId && typeof p.lat === 'number' && typeof p.lon === 'number' && !isNaN(p.lat) && !isNaN(p.lon);
                });
                if (validPoints.length !== points.length) {
                    console.warn('[sensortrigger] removed', points.length - validPoints.length, 'invalid points');
                    localStorage.setItem('sensortrigger_points', Ext.encode(validPoints));
                }
                me.triggerPointsStore.loadData(validPoints);
                console.log('[sensortrigger] loaded', validPoints.length, 'valid trigger points');
    ...
    addTriggerPointMarker: function(record) {
        ...
        if (typeof lat !== 'number' || typeof lon !== 'number' || isNaN(lat) || isNaN(lon)) {
            console.warn('[sensortrigger] invalid coordinates for', sensorId, lat, lon);
            return;
        }
        ...
        console.log('[sensortrigger] marker added for', sensorId, lat, lon);
    },
    ...
    renderAllTriggerMarkers: function() {
        ...
        for (var id in me.triggerMarkers) {
            if (me.triggerMarkers.hasOwnProperty(id)) me.triggerMarkers[id].remove();
        }
        me.triggerMarkers = {};
        // Добавляем маркеры только для валидных точек
